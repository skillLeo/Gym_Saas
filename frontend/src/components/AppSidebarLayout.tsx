...
import {
  ...
  Shield, Key, Bell, Search, ChevronDown, X, Menu,
} from 'lucide-react';
...
export default function AppSidebarLayout({ children, fullWidth = false }: Props) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className="px-4 h-16 flex items-center border-b border-gray-100 shrink-0">
  ...
  if (mode === 'topnav') {
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col">
        <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-xl h-16 flex items-center">
          <div className="max-w-7xl mx-auto px-4 md:px-6 w-full flex items-center gap-4">
            <Link href="/dashboard" className="flex items-center gap-2.5 shrink-0 mr-4">
              ...
              <span className="font-bold text-sm text-gray-900 hidden sm:block leading-none">My EXtreme <br /><span className="text-[#F87404]">Trainer</span></span>
            </Link>

            <nav className="hidden lg:flex items-center gap-0.5 flex-1">
              {NAV_MAIN.slice(0, 7).map(({ href, icon: Icon, label }) => {
                const active = isActive(href);
                return (
                  <Link key={href} href={href}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all ${active ? 'bg-[#F87404] text-white shadow-md shadow-[#F87404]/25' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}>
                    <Icon size={14} /> {label}
                  </Link>
                );
              })}
            ...
            <div className="flex items-center gap-2 ml-auto">
              {isOnTrial && (
                <Link href="/membership" className="hidden sm:flex items-center gap-1.5 bg-[#F87404]/10 text-[#F87404] text-xs font-bold px-3 py-1.5 rounded-full border border-[#F87404]/25">
                  🔥 {trialDays}d left
                </Link>
              )}
              {user?.is_admin && (
                <Link href="/admin" className="hidden md:flex items-center gap-1.5 bg-blue-50 text-blue-600 text-xs font-bold px-3 py-1.5 rounded-full border border-blue-200 hover:bg-blue-100 transition-colors">
              ...
              <Link href="/profile" className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl hover:bg-gray-100 transition-colors">
                <Avatar src={user?.avatar_url} name={user?.name ?? '?'} size={32} />
                <span className="hidden sm:block text-xs font-bold text-gray-900">{user?.name?.split(' ')[0]}</span>
              </Link>
            </div>
          </div>
        </header>

        <main className={`flex-1 pb-24 lg:pb-8 ${fullWidth ? '' : 'max-w-7xl mx-auto w-full px-4 md:px-6 py-6'}`}>
          {children}
        </main>
        <MobileBottom />
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-gray-100 fixed h-full z-30 shadow-sm">
        <SidebarContent />
      </aside>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <main className={`flex-1 pb-24 lg:pb-6 ${fullWidth ? '' : 'px-4 md:px-6 py-5'}`}>
          {children}
        </main>
      </div>
